...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/app-settings', async (req: any, res) => {
    try {
      ...
      const rewardPerAd = parseInt(getSetting('reward_per_ad', '50'));
      ...
      const taskRewardPerClick = parseFloat(getSetting('task_per_click_reward', '0.00002'));
  ...
  app.post('/api/ads/watch', authenticateTelegram, async (req: any, res) => {
    try {
      ...
      const rewardPerAdMGB = rewardPerAdSetting[0]?.settingValue ? parseInt(rewardPerAdSetting[0].settingValue) : 50;
  ...
  app.get('/api/admin/settings', authenticateAdmin, async (req: any, res) => {
    try {
      ...
      const taskPerClickRewardTON = parseFloat(getSetting('task_per_click_reward', '0.00002'));
      const walletChangeFeeTON = parseFloat(getSetting('wallet_change_fee', '0.0005'));
      
      // Convert TON to MGB for display (multiply by 5,000,000)
      const taskPerClickRewardMGB = Math.round(taskPerClickRewardTON * 5000000);
      const walletChangeFeeMGB = Math.round(walletChangeFeeTON * 5000000);
      
      // Return all settings in format expected by frontend
      res.json({
        dailyAdLimit: parseInt(getSetting('daily_ad_limit', '50')),
        rewardPerAd: parseInt(getSetting('reward_per_ad', '50')),
        affiliateCommission: parseFloat(getSetting('affiliate_commission', '10')),
        walletChangeFee: walletChangeFeeMGB, // Return as MGB
        minimumWithdrawal: parseFloat(getSetting('minimum_withdrawal', '0.1')),
        taskPerClickReward: taskPerClickRewardMGB, // Return as MGB
  ...
  app.put('/api/admin/settings', authenticateAdmin, async (req: any, res) => {
    try {
      ...
      // Convert MGB values to TON before saving (divide by 5,000,000)
  ...
  app.post('/api/advertiser-tasks/:taskId/increase-limit', authenticateTelegram, async (req: any, res) => {
    try {
      ...
      // Fetch dynamic minimum clicks from admin settings
      const minClicksSetting = await db.select().from(adminSettings).where(eq(adminSettings.settingKey, 'minimum_task_clicks')).limit(1);
      const minClicks = minClicksSetting[0]?.settingValue ? parseInt(minClicksSetting[0].settingValue) : 100;
      
      // Validation - minimum clicks based on admin settings
      if (!additionalClicks || additionalClicks < minClicks) {
        return res.status(400).json({
          success: false,
          message: `Minimum ${minClicks} additional clicks required`
        });
      }
